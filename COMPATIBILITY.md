...
- **MINOR**（例如 6.2.x → 6.3.0）：功能演进。**可能包含公开 API 的移除**，
  但仅限于经实测确认下游引用为零的类型，且每一项都会记录在下方清单中并附实测依据。
...
flatten-maven-plugin 处理后不含依赖声明，若使用 `implementation`，
整个 shaded 框架（含其内部依赖）会被打进你的模块 JAR，
并在运行时与服务器上已有的 UltiTools 冲突。

## 6.3.0 的移除清单

以下类型将在 6.3.0 移除。每一项都附有移除依据——
2026-08-10 对 `Modules/` 下 17 个独立下游 Git 仓库的引用量实测（排除测试目录）。
...
**建议等这两项都发布后再开始迁移。** 只等到 6.2.5 就动手的话，
迁移过去的命令会静默丢失参数级 tab 补全——新基类当前只在第一个参数位补全字面量。
