...
class TestUntrackedSurfaceFile:
    ...
    def test_a_completely_untracked_file_would_otherwise_look_clean_to_plain_git_diff(self, tmp_path):
        """Positive control for T-10-14's own premise: prove that WITHOUT the trackedness
        check, `git diff --exit-code` really does report a brand new untracked file as
        clean (exit 0) -- confirming the vulnerability this guard's check closes is real,
        not a hypothetical."""
        init_repo(tmp_path)
        (tmp_path / 'uat').mkdir()
        (tmp_path / 'uat' / 'surface.json').write_text(DEFAULT_SURFACE, encoding='utf-8')

        result = subprocess.run(['git', 'diff', '--exit-code', '--', 'uat/surface.json'],
...
class TestFirstTimeGeneration:
    ...
    def test_intent_to_add_with_no_real_baseline_passes(self, tmp_path):
        ...
        subprocess.run(['git', 'add', '-N', 'uat/surface.json'], cwd=tmp_path, check=True)
        bin_dir = stub_toolchain(tmp_path)
...
class TestByteIdentityDrift:
    ...
    def test_regeneration_matching_the_committed_baseline_is_clean(self, tmp_path):
        ...
        (tmp_path / 'uat').mkdir()
        (tmp_path / 'uat' / 'surface.json').write_text(DEFAULT_SURFACE, encoding='utf-8')
        subprocess.run(['git', 'add', 'uat/surface.json'], cwd=tmp_path, check=True)
    ...
    def test_hand_editing_the_committed_baseline_is_visible_to_plain_git_diff(self, tmp_path):
        """The other half of the guard's byte-identity claim: a hand edit to an already
        genuinely-tracked file (real staged content, not intent-to-add) is a real diff
        under plain git tooling -- proving the guard's underlying diff mechanism has a
        real signal to react to, not just a name-matching heuristic."""
        init_repo(tmp_path)
        (tmp_path / 'uat').mkdir()
        (tmp_path / 'uat' / 'surface.json').write_text(DEFAULT_SURFACE, encoding='utf-8')
        subprocess.run(['git', 'add', 'uat/surface.json'], cwd=tmp_path, check=True)
        ...
        dirty = subprocess.run(['git', 'diff', '--exit-code', '--', 'uat/surface.json'],
                                cwd=tmp_path, capture_output=True, text=True)
        ...
        (tmp_path / 'uat' / 'surface.json').write_text(DEFAULT_SURFACE, encoding='utf-8')
        clean = subprocess.run(['git', 'diff', '--exit-code', '--', 'uat/surface.json'],
                                cwd=tmp_path, capture_output=True, text=True)
...
class TestArguments:
    ...
    def test_module_name_defaults_to_the_checkout_directory_basename(self, tmp_path):
        ...
        subprocess.run(['git', 'add', 'uat/surface.json'], cwd=module_dir, check=True)
