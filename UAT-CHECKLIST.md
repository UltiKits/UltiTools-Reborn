...
| ultitools.boot.update-check.neg-none | `language: en` in config.yml; server has outbound access to the update-check endpoint; no newer framework or module version exists — `checkFrameworkUpdate`/`checkModuleUpdates` each catch their own request failures and leave both update collections empty, so a broken network path produces the exact same "up to date" line as a genuinely successful check with nothing to report; this row is only satisfied when the check actually ran | Restart the server and read the console output roughly one tick after startup completes | Console shows `[UltiTools-API] All plugins are up to date!`, with NO preceding `[UltiTools-API] Failed to check framework update:` or `[UltiTools-API] Failed to check update for <module>:` warning line — a query that silently failed is not the same as one that succeeded and found nothing | server | |
...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ultitools.remote.config-update-file | `ultipanel.capabilities.file-write: true`; at least one loaded module with an `@ConfigEntity`-registered config file | Send an `update_config` panel message with `fileName` set to that module's registered config path and `configData` holding a modified `{configEntry: value}` JSON map | The file on disk reflects the new values; a `config_update_response` with `success: true` is returned | server | |
...
| ultitools.remote.config-update-server-properties | `ultipanel.capabilities.file-write: true`; `ultipanel.capabilities.server-properties: false` (shipped default) | Send an `update_config` panel message with `fileName: "server.properties"` and `configData` holding a safe-key change (e.g. `motd`) | The write succeeds even though `server-properties` is disabled — confirming `update_config`'s server.properties branch is gated by `FILE_WRITE`, not `SERVER_PROPERTIES`, unlike the dedicated `server_properties` message type above | server | |
