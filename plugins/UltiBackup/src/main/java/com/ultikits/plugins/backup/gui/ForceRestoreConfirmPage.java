...
public class ForceRestoreConfirmPage extends BaseConfirmationPage {
    ...
    public ForceRestoreConfirmPage(Player viewer, BackupMetadata metadata, 
            BackupService backupService, Player target) {
        super(viewer, "force_restore_confirm", 
            UltiBackup.getInstance().i18n("backup.confirm.title"), 3);
    ...
    public static void open(Player viewer, BackupMetadata metadata, BackupService backupService) {
        ...
        if (target == null) {
            viewer.sendMessage(UltiBackup.getInstance().i18n("backup.message.player_offline")
                .replace("{PLAYER}", metadata.getPlayerName()));
            return;
        }
        
        new ForceRestoreConfirmPage(viewer, metadata, backupService, target).open();
    }
    ...
    protected void onConfirm(InventoryClickEvent event) {
        ...
        switch (result) {
            case SUCCESS:
                ...
                UltiBackup.getInstance().getLogger().warn(
