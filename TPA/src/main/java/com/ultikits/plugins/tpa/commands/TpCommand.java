...
public class TpCommand extends AbstractTabExecutor {
            ...
            case 1:
                switch (strings[0]) {
                    ...
                    default:
                        ...
                        tpaService.requestTpa(player, target, player);
                        player.sendMessage(MessagesUtils.info(String.format(Tpa.getInstance().i18n("tpa_tp_send_successfully"), target.getName())));
                        target.sendMessage(MessagesUtils.info(String.format(Tpa.getInstance().i18n("tpa_tp_enquire"), player.getName())));
                        target.sendMessage(MessagesUtils.info(Tpa.getInstance().i18n("tpa_accept_tip")));
                        target.sendMessage(MessagesUtils.info(Tpa.getInstance().i18n("tpa_reject_tip")));
                        return true;
                }
            default:
                return false;
