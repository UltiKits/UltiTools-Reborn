...
| `listener` | `member`, `event` (the first parameter's simple type name, when the handler takes one), `handler_priority`. One row per Bukkit `@EventHandler` method, not one row per `@EventListener` class — a class with four handler methods produces four listener rows. `gate` when applicable. |
