...
public class ProxyFactory {
     ...
     * <p>
     * The naming convention is owned by this class: callers that need to unwrap a proxy
     * must ask here rather than pattern-matching class names themselves. A previous
     * implementation detail leaked into {@code TaskManager}, which matched CGLIB's
     * {@code $$} infix and silently stopped recognising proxies when the engine changed.
     *
    ...
    public <T> T createProxy(Class<T> targetClass, T target) {
        try {
            Class<? extends T> proxyClass = new ByteBuddy()
                    ...
                    // clone() is excluded separately: it is a protected member of java.lang.Object,
                    // and the handler's method.setAccessible(true) throws InaccessibleObjectException
                    // on JDK 17+ because module java.base does not open java.lang to the unnamed module.
