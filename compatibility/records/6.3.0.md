...
(28 of them, across 12 files) tolerate a `LinkageError` — was rejected: `UltiTools` itself is never
proxied, so the tolerance would have had no beneficiary, and widening a linkage-error catch at
every reflection site risks masking a genuinely broken classpath for a *required* dependency, not
just an absent optional one. Removing the one offending signature fixes the actual defect at its
source instead of teaching 28 call sites to shrug at a class of error that, for every other type on
