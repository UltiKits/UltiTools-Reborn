...
| Search | Result | Control |
|---|---|---|
| `grep -rlI "CloudAuthManager" --include=*.java` across every `Modules/` repository (15 active modules, discontinued `UltiBot`, non-product `ultikits-module-parent`) plus `Tooling/UltiTools-External-Example` | **0** files | **173** files for `UltiToolsPlugin` over the same 17 repositories |
...
Replacement: `EconomyUtils.getEconomy()` — the pre-existing façade all six economy-consuming
modules already call, unaffected by this change (its own signature is unchanged).
...
`EconomyUtils` — the single choke point all six consuming modules already call — behind a new
