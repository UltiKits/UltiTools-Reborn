...
| ultitools.upm.uninstall | Uninstall a plugin: unload it through the framework's full unload path (`PluginManager#unregister`), delete its JAR, and report the real outcome — success only once the JAR is deleted, otherwise a failure naming the JAR still on disk | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
