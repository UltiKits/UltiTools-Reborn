...
| ultitools.upm.uninstall | Uninstall a plugin, refused with nothing changed while an update or uninstall of the same module is running: unload it through the framework's full unload path (`PluginManager#unregister`), delete every JAR whose `plugin.yml` `name:` matches, and report the real outcome — success only once every such JAR is deleted; otherwise a failure naming each JAR still on disk, a notice that the module was unloaded but no JAR was found, or (when the module's own unload threw) an unload-error line followed by the JAR outcome | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
| ultitools.upm.update | Update one named module as a staged transaction whose last step is the next boot: download into `plugins/UltiTools/.upm-staging/`, validate the download's `plugin.yml` against the module's identify-string, name and the catalogue version, record the transaction in a journal file in that directory, move the module's older JARs aside, move the new JAR in, and stop. The old JARs and the journal stay until the next start, which confirms the update if the module loaded and rolls it back if it did not; the reply says so. It is refused with nothing changed when an update or uninstall of the same module is running, when the staging and modules folders are not on the same file system (a JAR is only ever renamed atomically, never copied), or when a JAR of the module newer than the catalogue version is already present. A failure changes nothing, except that a set-aside JAR that cannot be moved back is named with the exact original path to restore it to; a failed move names its cause; set-aside JARs that cannot be deleted after a successful update are reported as an inert leftover. At the next start, before modules load, the framework moves back the JARs named by each journal an interrupted update left behind and deletes stale partial downloads; a journal whose JARs could not all be moved back is kept and retried at the next start. A set-aside JAR that no surviving journal refers to is reported by absolute path as a leftover the operator can delete, never moved back; one whose journal is still there is reported as belonging to it and must not be deleted. After the modules have loaded, every journal awaiting confirmation is resolved: the module loaded, so its old JAR and the journal are deleted; or it did not, so the installed JAR is deleted, the old one is moved back, and a SEVERE line names the module, the version it was rolled back to, and that it returns at the next restart. On Windows a loaded module's JAR cannot be moved, so the update rolls back and says so; stop the server or unload the module to update it there | command | `/upm update <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#updatePlugin |
| ultitools.upm.update-all | Update every module with an available update in one command, one staged transaction per module, with per-module outcome lines and a summary that counts successes, failures and refused (already-running) updates; the restart instruction appears only in that summary, after every module has finished | command | `/upm update all` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#updateAllPlugins |
| ultitools.upm.update.boot-confirmation | An update installed by `/upm update` is confirmed or rolled back by the next start. After the modules load, the framework checks whether each module with an update awaiting confirmation is among the loaded ones, matched by the identify-string its journal records rather than by its display name. While an update waits for that restart, a second update of the same module is refused and changes nothing; a rollback that could not finish is finished at the next start rather than read as an update to confirm. If it is, the version it replaced is deleted and the journal with it. If it is not, the installed JAR is deleted, the previous version is moved back into the modules folder, and the console carries a SEVERE line naming the module, the version restored, that the module is NOT available in this session, and that a restart loads it again. A failure inside this step is logged and never stops the server | boot | (automatic, after module loading) | none | server | admin | brief | PluginInstallUtils#confirmUpdatesAfterBoot |
