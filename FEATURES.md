...
| ultitools.capability.logs | Whether the panel receives the live console log stream; ships enabled — the switch decides only whether the stream is delivered, never a control over it (#495) | gate | `ultipanel.capabilities.logs` in config.yml | n/a | n/a | admin | brief | Capability#LOGS |
...
| ultitools.config.config.ultipanel.capabilities.logs | Whether the panel receives the live console log stream; ships enabled (#495) | config | `src/main/resources/config.yml: ultipanel.capabilities.logs (default: true)` | n/a | n/a | admin | brief | Capability#LOGS |
...
| ultitools.config.log-stream-wording | The shipped `config.yml` resource's `ultipanel.capabilities.logs` comment and `ultipanel.logging.batch.interval` comment, in both English and Chinese, and `Capability#LOGS`'s own description compiled into the jar, state that the `logs` switch decides whether the live log stream is delivered — never that the panel controls the stream — and that the batch interval is read from this file each time a panel connection opens, not once at server start; `config-example.yml` carries the same corrected `logs` sentence (#495 fixed) | gate | `src/main/resources/config.yml` comments for `ultipanel.capabilities.logs`/`ultipanel.logging.batch.interval`, `config-example.yml`'s `ultipanel.capabilities.logs` comment, and `Capability#LOGS`'s compiled description, read directly from the built jar or from source | n/a | n/a | admin | brief | Capability#LOGS, LogStreamManager#loadBatchConfiguration |
