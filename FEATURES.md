...
| ultitools.upm.update | Update one named module as a staged transaction: download into `plugins/UltiTools/.upm-staging/`, validate it as a JAR of that module at the catalogue's latest version, move the module's older JARs aside, move the new JAR in, then delete the set-aside JARs. It is refused with nothing changed when an update or uninstall of the same module is running, when the staging and modules folders are not on the same file system (a JAR is only ever renamed atomically, never copied), or when a JAR of the module newer than the catalogue version is already present. A failure changes nothing, except that a set-aside JAR that cannot be moved back is named with the exact original path to restore it to; a failed move names its cause; set-aside JARs that cannot be deleted after a successful update are reported as an inert leftover. At the next start, before modules load, the framework restores a set-aside JAR an interrupted update left behind and deletes stale partial downloads. On Windows a loaded module's JAR cannot be moved, so the update rolls back and says so; stop the server or unload the module to update it there | command | `/upm update <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#updatePlugin |
