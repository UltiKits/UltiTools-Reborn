...
| ultitools.upm.uninstall | Uninstall a module: unload every loaded instance of it through the framework's full unload path — so its scheduled tasks, cached beans, completers and EventBus handlers stop at once rather than at the next restart — then delete every JAR whose `plugin.yml` name matches, and report what actually happened on disk: success only once every such JAR is gone, the JARs still there by name when one could not be deleted, and a distinct notice when the module was unloaded but no JAR of it was found. A JAR whose metadata cannot be read is reported rather than passed over, since it loads the module again once it is readable | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
