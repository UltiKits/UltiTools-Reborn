...
| ultitools.upm.update.boot-confirmation | An update installed by `/upm update` is confirmed or rolled back by the next start. After the modules load, the framework checks whether each module with an update awaiting confirmation is among the loaded ones. If it is, the version it replaced is deleted and the journal with it. If it is not, the installed JAR is deleted, the previous version is moved back into the modules folder, and the console carries a SEVERE line naming the module, the version restored, that the module is NOT available in this session, and that a restart loads it again. A failure inside this step is logged and never stops the server | boot | (automatic, after module loading) | none | server | admin | brief | PluginInstallUtils#confirmUpdatesAfterBoot |
