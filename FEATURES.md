...
| ultitools.upm.uninstall | Uninstall a module: unload every loaded instance of it through the framework's full unload path — so its scheduled tasks, cached beans, completers and EventBus handlers stop at once rather than at the next restart — then delete every JAR whose `plugin.yml` name matches, and report what actually happened on disk: success only once every such JAR is gone, the JARs still there by name when one could not be deleted, and a distinct notice when the module was unloaded but no JAR of it was found. A loaded module is asked which JAR it came from, so its own JAR is deleted even when it carries no `plugin.yml` — modules are identified by `@UltiToolsModule`, not by metadata. An entry the command could not read at all, or that carries no metadata to read, is reported as such. One limit is stated rather than closed: a second copy whose `plugin.yml` declares a different module than the loaded copy's JAR is not detected, because deciding that would require loading classes out of the JAR, which this framework deliberately does not do here — so the reply says what was removed and what could not be determined, never that no copy of the module remains — never guessed at from its file name — and an entry that opened and declares another module, or no module, is left alone in silence; a modules folder that cannot be listed is reported as that rather than as an absent JAR | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
