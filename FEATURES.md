...
| ultitools.upm.uninstall | Uninstall a module: resolve which module the operator named — by its runtime name or by a name one of its JARs declares — unload every loaded instance of it through the framework's full unload path, so its scheduled tasks, cached beans, completers and EventBus handlers stop at once rather than at the next restart, then delete the JARs identified as its own and report what actually happened on disk. A loaded module is asked which JAR it came from, so its own JAR is deleted even when it carries no `plugin.yml` — modules are identified by `@UltiToolsModule`, not by metadata. No JAR another loaded module was loaded from is ever deleted. An entry the command could not identify — it would not open, its `plugin.yml` is not valid YAML, or it declares no module — is reported as such rather than passed over or called a copy; an entry that opened and declares another module is left alone in silence; and a modules folder that cannot be listed is reported as that rather than as an absent JAR. Success names the files it deleted and never claims that no copy of the module remains: a second copy whose `plugin.yml` declares a different module than the loaded copy's JAR is not detected, because deciding that would require loading classes out of the JAR, which this framework deliberately does not do here (#516) | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultitools.language.file-preserve | An `UltiToolsPlugin` module's extracted `lang/<code><ext>` file that has been customised since extraction (its bytes no longer match the recorded provenance hash, or no provenance was ever recorded and its bytes differ from the jar's) is never overwritten; only individual keys that provably lost a placeholder relative to the jar's bundled value are resolved from the jar instead, each logged once as a WARN naming the module, file and key (D-05/D-06, #441; brace-token dialect added #524). Two independent detectors, evaluated separately: `String.format` placeholder arity (distinct argument positions, `%s`/`%d`-style — a SYMMETRIC count comparison, either side moving is a mismatch) and `{0}`/`{PLAYER}`-style brace tokens (substituted with plain `String#replace`, never `Formatter` — a ONE-DIRECTIONAL presence check that fires only when a token the jar's value has is absent from the disk value, never on an operator's own added token or reworded text) | gate | any loaded module's `plugins/UltiTools/pluginConfig/<module>/lang/<code><ext>` file, hand-edited after extraction, with the module jar later shipping either a changed `%s`/`%d` placeholder count or a `{0}`/`{PLAYER}`-style token the hand-edited value lacks, for one of the edited keys — observed on the module's next start | n/a | n/a | admin | brief | UltiToolsPlugin#applyPlaceholderArityOverride, UltiToolsPlugin#missingBracePlaceholder |
...
| ultitools.capability.logs | Allow the panel to receive the live console log stream — never to control, pause or resume it; ships enabled (#495) | gate | `ultipanel.capabilities.logs` in config.yml | n/a | n/a | admin | brief | Capability#LOGS |
...
| ultitools.remote.log-stream-wording | The shipped `config.yml` resource's `ultipanel.capabilities.logs` comment and `ultipanel.logging.batch.interval` comment, in both English and Chinese, and `Capability#LOGS`'s own description compiled into the jar, state that the `logs` switch decides whether the live log stream is delivered — never that the panel controls, pauses, resumes, starts or stops it (matching the declared-vs-delivered corrections above) — and that the batch interval's applied value comes from the framework's currently loaded configuration each time a panel connection opens — not a direct disk re-read, so an on-disk edit needs `ul reload` before the next connection picks it up, and not fixed once at server start; `config-example.yml` carries the same corrected `logs` sentence (#495 fixed) | gate | `src/main/resources/config.yml` comments for `ultipanel.capabilities.logs`/`ultipanel.logging.batch.interval`, `config-example.yml`'s `ultipanel.capabilities.logs` comment, and `Capability#LOGS`'s compiled description, read directly from the built jar or from source | n/a | n/a | admin | brief | Capability#LOGS, LogStreamManager#loadBatchConfiguration |
...
| ultitools.config.config.ultipanel.capabilities.logs | Whether the panel may receive the live console log stream; ships enabled (#495) | config | `src/main/resources/config.yml: ultipanel.capabilities.logs (default: true)` | n/a | n/a | admin | brief | Capability#LOGS |
