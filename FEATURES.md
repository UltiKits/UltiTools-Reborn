...
| ultitools.upm.uninstall | Uninstall a plugin: unload it through the framework's full unload path (`PluginManager#unregister`), delete every JAR whose `plugin.yml` `name:` matches, and report the real outcome — success only once every such JAR is deleted; otherwise a failure naming each JAR still on disk, a notice that the module was unloaded but no JAR was found, or (when the module's own unload threw) an unload-error line followed by the JAR outcome | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
| ultitools.upm.update | Update one named module as a staged transaction: download into `plugins/UltiTools/.upm-staging/`, validate it as a JAR of that module at the catalogue's latest version, move the module's older JARs aside, move the new JAR in, then delete the set-aside JARs; any failure changes nothing (older JARs moved back) and is reported, an update of a module whose update is already running is refused, and set-aside JARs that cannot be deleted are reported as an inert leftover | command | `/upm update <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#updatePlugin |
| ultitools.upm.update-all | Update every module with an available update in one command, one staged transaction per module, with per-module outcome lines and a summary that counts successes, failures and refused (already-running) updates | command | `/upm update all` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#updateAllPlugins |
