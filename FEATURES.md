...
| ultitools.upm.uninstall | Uninstall a module: unload every loaded instance of it through the framework's full unload path — so its scheduled tasks, cached beans, completers and EventBus handlers stop at once rather than at the next restart — then delete every JAR whose `plugin.yml` name matches, and report what actually happened on disk: success only once every such JAR is gone, the JARs still there by name when one could not be deleted, and a distinct notice when the module was unloaded but no JAR of it was found. An entry the command could not read at all is reported as such — never guessed at from its file name — and an entry that opened and declares another module, or no module, is left alone in silence; a modules folder that cannot be listed is reported as that rather than as an absent JAR | command | `/upm uninstall <plugin>` | none (requireOp=true) | both | admin | brief | PluginInstallCommands#uninstallPlugin |
